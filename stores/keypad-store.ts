...
import type { FirmwareProfile, HardwareInfo, HidDevice, KeyAssignment } from "@/lib/sikai-keypad";
...
function assignmentsDiffer(current: KeyAssignment[] | null, next: KeyAssignment[]) {
  return Boolean(current?.some((assignment, index) => (
    assignment.modifier !== next[index]?.modifier || assignment.keyCode !== next[index]?.keyCode
  )));
}
...
function editorDefaults() {
  return {
    ...
    readMessage: "Current assignments will load automatically after connecting.",
    writeStatus: "idle" as const,
    writeMessage: "Connect the keypad to load its current assignments.",
    profile: null,
    profileStatus: "idle" as const,
    profileMessage: "Firmware details will load automatically after connecting.",
...
let disconnectListener: ((event: Event & { device?: HidDevice }) => void) | null = null;
...
function attachDisconnectListener(keypad: SikaiKeypad) {
  ...
  disconnectListener = (event) => {
    ...
    useKeypadStore.setState({
      ...
      connectionMessage: "Keypad disconnected. The assignment and RGB editors were reset.",
    });
  };
...
export const useKeypadStore = create<KeypadState>((set, get) => ({
  ...
  connect: async () => {
    if (!get().supported) {
      set({
        ...
        connectionMessage: "WebHID is unavailable here. Open this page in Chrome or Edge on Windows.",
      });
      return;
    }
    ...
    set({
      ...
      connectionMessage: "Choose the SIKAI USB Keyboard shown in the browser prompt.",
    });
    try {
      const selected = await SikaiKeypad.request();
      if (!selected) {
        set({ connectionStatus: "idle", connectionMessage: "Nothing was selected. You can try again safely." });
        return;
      }
      ...
      set({
        ...
        connectionMessage: "Connected. Loading current assignments and supported lighting settings…",
      });
      await get().reloadAssignments();
      await get().detectFirmware();
      set({ connectionMessage: "Finished loading available keypad settings. Changes are saved only when you press Apply." });
    } catch (error) {
      set({
        connectionStatus: "error",
        connectionMessage: error instanceof Error ? error.message : "The browser could not open the device.",
      });
    }
  },
  ...
  reloadAssignments: async () => {
    ...
    set({ readStatus: "reading", packets: [], readMessage: "Loading the current layer 1 assignments…" });
    try {
      const result = await keypad.readAssignments((packets) => set({ packets }));
      set({
        ...
        writeStatus: "idle",
        writeMessage: "Current assignments loaded. Edit either key, then apply your changes.",
      });
    } catch (error) {
      set({
        readStatus: "error",
        readMessage: error instanceof Error ? error.message : "The current assignments could not be loaded.",
      });
    }
  },
  ...
  updateAssignment: (index, patch) => {
    set((state) => {
      const assignments = state.assignments.map((assignment, assignmentIndex) => (
        assignmentIndex === index ? { ...assignment, ...patch } : assignment
      ));
      return {
        assignments,
        assignmentsChanged: assignmentsDiffer(state.currentAssignments, assignments),
        writeStatus: "idle",
        writeMessage: "Review the new shortcuts, then press Apply to save and verify them.",
      };
    });
  },
  ...
  applyAssignments: async () => {
    ...
    set({ writeStatus: "writing", writeMessage: "Writing the changed key records, saving once, then reading them back…" });
    try {
      ...
      set({
        ...
        assignmentsChanged: false,
        writeStatus: "success",
        writeMessage: "Saved and verified from the keypad. The new shortcuts are active in onboard memory.",
      });
    } catch (error) {
      set({
        writeStatus: "error",
        writeMessage: error instanceof Error ? error.message : "The keypad could not save the new shortcuts.",
      });
    }
  },
  ...
  detectFirmware: async () => {
    ...
    set({ profileStatus: "reading", profile: null, profileMessage: "Reading the keypad firmware identity…" });
    try {
      const profile = await keypad.readFirmwareProfile();
      set({
        profile,
        profileStatus: "success",
        profileMessage: profile.protocol === 0x0a
      ...
      if (profile.protocol === 0x0a) {
        set({ rgbStatus: "idle", rgbMessage: "Loading the current RGB table…" });
        try {
          ...
          set({
            ...
            rgbStatus: "success",
            rgbMessage: "Current layer 1 lighting was loaded from the keypad.",
          });
        } catch (error) {
          set({
            rgbStatus: "error",
            rgbMessage: error instanceof Error ? error.message : "The current RGB table could not be loaded.",
          });
        }
      } else if (profile.protocol === 0x00) {
        set({
          rgbStatus: "idle",
          rgbMessage: "Legacy protocol 0x00 supports RGB saving, but does not expose a readable current-lighting record. Choose a setting to replace it.",
        });
      } else {
        set({ rgbStatus: "idle", rgbMessage: `RGB is disabled because protocol ${hex(profile.protocol, 2)} is not supported.` });
      }
    } catch (error) {
      set({
        profileStatus: "error",
        profileMessage: error instanceof Error ? error.message : "The firmware identity could not be read.",
        rgbStatus: "error",
        rgbMessage: "RGB writing stays disabled until the firmware can be identified.",
      });
    }
  },
  ...
  applyRgb: async () => {
    ...
    set({
      rgbStatus: "writing",
      rgbMessage: profile.protocol === 0x0a ? "Reading and preserving all three RGB layers…" : "Saving the corrected legacy RGB record…",
    });
    try {
      ...
      set({ rgbStatus: "error", rgbMessage: error instanceof Error ? error.message : "The RGB setting could not be saved." });
    }
  },
  ...
  copyPackets: async () => {
    ...
    set({ readMessage: `Copied ${packets.length} raw configuration packets.` });
  },
}));
