    ...
    <article className={`diagnosticCard ${readStatus}`}>
      <div className="diagnosticCopy">
        ...
        <div className="diagnosticActions">
          <button className="acidButton" onClick={reloadAssignments} disabled={!connected || readStatus === "reading"}>
            ...
            {assignments.map((assignment, index) => (
              <div className="keyEditor" key={index}>
                <div className="keyEditorHeading">
                  ...
                  <small>CURRENT: {shortcutLabel(currentAssignments[index])}</small>
                </div>
                    ...
                    <label key={mask}>
                      <input
                        ...
                        onChange={(event) => updateAssignment(index, {
                          modifier: event.target.checked ? assignment.modifier | mask : assignment.modifier & ~mask,
                        })}
                      />
                      <span>{label}</span>
                    </label>
                ...
                <strong className="shortcutPreview">{shortcutLabel(assignment)}</strong>
              </div>
            ))}
